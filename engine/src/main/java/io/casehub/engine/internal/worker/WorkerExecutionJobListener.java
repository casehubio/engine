...
public class WorkerExecutionJobListener implements JobListener {
    ...
    private void maybeRescheduleJob(JobExecutionContext context) {
        ...
        workerExecutionRecoveryService.loadOrRestoreCaseInstance(caseId)
                ...
                .subscribe().with(
                        ...
                        ex -> LOG.errorf(ex, "Failed to reschedule worker %s for case %s", workerId, caseHubInstanceUuid)
                );
    }
    ...
    private void rescheduleJob(JobExecutionContext context, RetryPolicy retryPolicy) {
        ...
        workerExecutionScheduler.scheduleOrRescheduleAsync(job, trigger)
                .subscribe().with(
