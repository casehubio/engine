...
public class WorkerRetryTest {
    ...
    public void test() {
        ...
        await().atMost(30, TimeUnit.SECONDS).untilAsserted(() -> {
            assertTrue(CaseHubBean.runCount.get() >= 3,
                    "Expected at least 3 attempts (2 failures + 1 success), got " + CaseHubBean.runCount.get());
        });
    }
                                ...
                                public Map<String, Object> apply(Map<String, Object> stateContext) {
                                    ...
                                    return Map.of(
                                            "processedDocument", Map.of(
                                                    "id", stateContext.get("documentId"),
                                                    "content", "Processed content for document " + stateContext.get("documentId")
                                            ),
                                            "status", "processed"
                                    );
                                }
