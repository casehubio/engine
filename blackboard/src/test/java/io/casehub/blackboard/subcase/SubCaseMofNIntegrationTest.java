...
/**
 * Tests the M-of-N grouped SubCase path — only 2 of 3 children need to complete before the parent
 * resumes. The third child must remain RUNNING (KEEP policy, not cancelled).
 *
...
class SubCaseMofNIntegrationTest {
  ...
  @Inject MofNParentBean parentCase;
  ...
  void twoOfThree_firstTwoComplete_parentResumes_thirdKeepsRunning() {
    ...
    // Parent must resume after 2 complete — threshold reached
    await()
        .atMost(15, TimeUnit.SECONDS)
        .until(
            () -> {
    ...
    assertThat(caseInstanceCache.get(parentId).getState())
        .as("parent must leave WAITING once 2-of-3 threshold is reached")
        .isNotEqualTo(CaseStatus.WAITING);

    // Third child must still be RUNNING — KEEP policy means it is not cancelled
    CaseInstance thirdChild = caseInstanceCache.get(childIds.get(2));
    assertThat(thirdChild).isNotNull();
    assertThat(thirdChild.getState())
        .as("third child must still be RUNNING — KEEP policy does not cancel it")
   ...
   * child from auto-completing before the test simulates completion via the listener.
   */
  @ApplicationScoped
  ...
  /**
   * Parent that spawns 3 grouped children ({@code totalInGroup=3, requiredCount=2}) and waits for
   * any 2 of them. Uses {@link OnThresholdReached#KEEP} so the surviving child continues running.
   */
  @ApplicationScoped
  public static class MofNParentBean extends CaseHub {
