...
class ActionGateHandlerTest {
  ...
  private static final String TENANCY_ID =
  ...
  void onActionGateSchedule_noExpiresAt_whenExpiresInIsNull() {
    final UUID caseId = UUID.randomUUID();
    final GateRequired gate =
        new GateRequired("Confirm action", true, null, null, null, null, null);
  ...
  void completionApplier_cancelled_publishesRejectedEvent() {
    final UUID caseId = UUID.randomUUID();
    final WorkItem workItem = WorkItem.builder().callerRef(GateRef.encode(caseId, 30L)).build();
  ...
  void completionApplier_expired_publishesExpiredEvent() {
    final UUID caseId = UUID.randomUUID();
    final WorkItem workItem = WorkItem.builder().callerRef(GateRef.encode(caseId, 40L)).build();
  ...
  void completionApplier_escalated_publishesExpiredEvent() {
    final UUID caseId = UUID.randomUUID();
    final WorkItem workItem = WorkItem.builder().callerRef(GateRef.encode(caseId, 42L)).build();
    ...
    await()
        ...
        .until(() -> GateEventRecorder.expiredEvents.stream().anyMatch(e -> e.gateId() == 42L));
