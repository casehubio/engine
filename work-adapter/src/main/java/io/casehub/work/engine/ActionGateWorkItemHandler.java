 ...
 *
 * <p>Implements {@link ActionGateScheduler} — discovered by the engine runtime via CDI {@code
 * Instance<ActionGateScheduler>}. Creates a WorkItem with:
 *
 * <ul>
...
public class ActionGateWorkItemHandler implements ActionGateScheduler {
  ...
  private static String buildPayload(final ActionGateScheduleRequest event) {
    ...
    try {
      ...
      LOG.warning(
          "Failed to serialize gate payload for gateId="
              + event.gateId()
              + " — using null: "
              + e.getMessage());
      return null;
  ...
  public void schedule(final ActionGateScheduleRequest event) {
    ...
    if (event.gateRequired().quorum() != null) {
      ...
      LOG.info(
          "Gate multi-instance group created: caseId="
              + event.caseId()
              + " gateId="
              + event.gateId()
              + " instances="
