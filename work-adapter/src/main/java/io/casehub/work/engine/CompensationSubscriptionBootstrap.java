...
public class CompensationSubscriptionBootstrap {
  ...
  @Inject Instance<SubscriptionStore> subscriptionStoreInstance;

  @Inject Instance<EventTypeRegistry> eventTypeRegistryInstance;
  ...
  void onStartup(@Observes StartupEvent event) {
    ...
    Set<String> existing =
        store
            ...
            .filter(
                s ->
                    s.eventType().startsWith(COMPENSATION_PREFIX)
                        || s.eventType().startsWith(WORK_COMPENSATION_PREFIX))
            .map(Subscription::eventType)
            .collect(Collectors.toSet());

    registerWorkSubscription(
        store,
        existing,
        "started",
        "assigneeId",
        "WorkItem compensation started",
    ...
    registerWorkSubscription(
        ...
        "completed",
        "assigneeId",
        "WorkItem compensation completed",
        NotificationSeverity.INFO);

    registerCaseSubscription(
        store,
        existing,
        "started",
  ...
  private void registerWorkSubscription(
    ...
    try {
      ...
      LOG.warnf(
          "Failed to register compensation subscription for %s: %s", eventType, e.getMessage());
    }
  }

  private void registerCaseSubscription(
      SubscriptionStore store,
      Set<String> existing,
      String suffix,
      String titlePattern,
