...
public class TrustConsolidationPhase implements ConsolidationPhase {
  ...
  @Inject
  public TrustConsolidationPhase(
  ...
  TrustConsolidationPhase(
      MindMapStore mindMapStore, LedgerEntryRepository ledgerRepo, TrustEvolutionConfig config) {
    this.mindMapStore = mindMapStore;
  ...
  public void run(String tenantId, List<String> subgraphPriority) {
    ...
    Optional<String> peopleSubgraphId =
        mindMapStore.listSubgraphs(tenantId).stream()
  ...
  private void processOverlayNode(
      MindMapNode overlay,
      Map<String, MindMapNode> sharedNodes,
      TrustScoreComputer computer,
      Instant now,
      String tenantId) {
    ...
    List<LedgerEntry> entries = ledgerRepo.findByActorId(targetId, Instant.EPOCH, now, tenantId);
    if (entries.isEmpty()) return;

    Map<UUID, List<LedgerAttestation>> attestationMap = new HashMap<>();
    for (LedgerEntry entry : entries) {
      List<LedgerAttestation> allAttestations =
          ledgerRepo.findAttestationsByEntryId(entry.id, tenantId);
      List<LedgerAttestation> filtered =
          allAttestations.stream().filter(a -> observerId.equals(a.attestorId)).toList();
      if (!filtered.isEmpty()) {
    ...
    String previousRendered =
        overlay.property(OverlayTrustPropertyModel.TRUST_LAST_RENDERED).orElse(null);
    double previousScore = previousRendered != null ? Double.parseDouble(previousRendered) : -1.0;
    if (Math.abs(score.trustScore() - previousScore)
        > config.consolidation().significantChangeThreshold()) {
      properties.put(
          OverlayTrustPropertyModel.TRUST_LAST_RENDERED, String.valueOf(score.trustScore()));
    }

    mindMapStore.updateNode(
        overlay.id(), NodeUpdate.empty().withPropertiesToSet(properties), tenantId);
  }
  ...
  private DecayFunction buildDecayFunction() {
    ...
    return (ageInDays, verdict) -> {
      int effectiveHalfLife =
          (verdict == AttestationVerdict.FLAGGED || verdict == AttestationVerdict.CHALLENGED)
