...
public abstract class CaseQueueEntryStoreContractTest {

  protected abstract CaseQueueEntryStore store();

  protected CaseQueueEntry newEntry(UUID caseId, UUID viewId, String tenancyId) {
  ...
  void save_roundTripsAllFields() {
    ...
    entry.setClaimedAt(Instant.parse("2026-09-23T10:00:00Z"));
    entry.setEscalatedAt(Instant.parse("2026-09-23T11:00:00Z"));
    entry.setPreviousViewId(UUID.randomUUID());
    entry.setPreviousViewName("prev-queue");
    store().save(entry);

    CaseQueueEntry found = store().findById(entry.getId()).orElseThrow();
    assertThat(found.getCaseId()).isEqualTo(entry.getCaseId());
    assertThat(found.getTenancyId()).isEqualTo(entry.getTenancyId());
    assertThat(found.getViewId()).isEqualTo(entry.getViewId());
    assertThat(found.getViewName()).isEqualTo(entry.getViewName());
    assertThat(found.getStatus()).isEqualTo(entry.getStatus());
    assertThat(found.getAssignedTo()).isEqualTo("user-1");
    assertThat(found.getClaimedAt()).isEqualTo(Instant.parse("2026-09-23T10:00:00Z"));
    assertThat(found.getEscalatedAt()).isEqualTo(Instant.parse("2026-09-23T11:00:00Z"));
    assertThat(found.getPreviousViewId()).isEqualTo(entry.getPreviousViewId());
    assertThat(found.getPreviousViewName()).isEqualTo("prev-queue");
    assertThat(found.getCreatedAt()).isEqualTo(entry.getCreatedAt());
  }
}
