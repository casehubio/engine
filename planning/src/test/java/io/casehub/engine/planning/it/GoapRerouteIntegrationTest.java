...
@QuarkusTest
class GoapRerouteIntegrationTest {

  @Inject CaseInstanceCache cache;
  @Inject RerouteGoapBean rerouteBean;

  @BeforeEach
  void setUp() {
    RerouteGoapBean.executionOrder.clear();
    RerouteGoapBean.assessAttempts.set(0);
  }

  @Test
  void goapStrategy_continuesAfterFailureAndReroute() {
    UUID caseId = rerouteBean.startCase(Map.of("trigger", true));
    ...
    await()
        ...
        .untilAsserted(
            () -> {
              ...
              assertThat(RerouteGoapBean.executionOrder)
                  .as("GOAP plan should complete via rerouted agent after first failure")
                  .containsSubsequence("analyse", "assess-failed", "assess-ok", "report");
            });
  }

  @ApplicationScoped
  public static class RerouteGoapBean extends CaseHub {
    static final List<String> executionOrder = new CopyOnWriteArrayList<>();
    static final AtomicInteger assessAttempts = new AtomicInteger();
    ...
    public CaseDefinition getDefinition() {
      ...
      WorkerFunction.Sync<Map, Map> assessFn =
          new WorkerFunction.Sync<>(
              Map.class,
              Map.class,
              (input, scope) -> {
                if (assessAttempts.getAndIncrement() == 0) {
                  executionOrder.add("assess-failed");
                  return WorkerResult.failed("simulated failure");
                }
                executionOrder.add("assess-ok");
      ...
      return CaseDefinition.builder()
          ...
          .name("GOAP Reroute Test")
          .version("1.0.0")
          .planningStrategy("goap")
          .capabilities(capAnalyse, capAssess, capReport)
          .workers(wAnalyse, wAssess1, wAssess2, wReport)
          .bindings(
              Binding.builder()
                  .name("analyse")
                  .capability(capAnalyse)
