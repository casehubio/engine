...
@QuarkusTest
class AdaptiveStrategyIntegrationTest {

  @Inject CaseInstanceCache cache;
  @Inject AdaptiveReplanBean adaptiveBean;

  @BeforeEach
  void setUp() {
    AdaptiveReplanBean.executionOrder.clear();
  }

  @Test
  void adaptiveStrategy_replansWhenStepOutputChangesOptimalPath() {
    UUID caseId = adaptiveBean.startCase(Map.of("trigger", true));
    ...
    await()
        ...
        .untilAsserted(
            () -> {
              ...
              assertThat(AdaptiveReplanBean.executionOrder)
                  .as(
                      "Adaptive should pick fastTrack (cost 0.5) over assess (cost 2.0) "
                          + "after analyse produces lowRisk=true")
                  .containsExactly("analyse", "fastTrack", "report");
            });
  }

  @ApplicationScoped
  public static class AdaptiveReplanBean extends CaseHub {
    ...
    public CaseDefinition getDefinition() {
      ...
      Capability capFastTrack =
          Capability.builder().name("fastTrack").inputSchema(".").outputSchema(".").build();
              ...
              .function(
                  new WorkerFunction.Sync<>(
                      ...
                      (input, scope) -> {
                        ...
                        return WorkerResult.of(Map.of("analysisResult", true, "lowRisk", true));
                      }))
              .build();
      ...
      Worker wFastTrack =
          Worker.builder()
              .name("worker-fastTrack")
              .capabilityName("fastTrack")
              .function(
                  new WorkerFunction.Sync<>(
                      Map.class,
                      Map.class,
                      (input, scope) -> {
                        executionOrder.add("fastTrack");
                        return WorkerResult.of(Map.of("riskAssessment", true));
                      }))
              .build();
      ...
      GoapAction actAssess =
          new GoapAction(
              "assess", Map.of("analysisResult", true), Map.of("riskAssessment", true), 2.0);
      GoapAction actFastTrack =
          new GoapAction(
              "fastTrack",
              Map.of("analysisResult", true, "lowRisk", true),
      ...
      return CaseDefinition.builder()
          ...
          .capabilities(capAnalyse, capAssess, capFastTrack, capReport)
          .workers(wAnalyse, wAssess, wFastTrack, wReport)
          .bindings(
              Binding.builder()
                  .name("analyse")
                  .capability(capAnalyse)
              ...
              Binding.builder()
                  ...
                  .on(new ContextChangeTrigger(".trigger == true"))
                  .build())
          .goapActions(List.of(actAnalyse, actAssess, actFastTrack, actReport))
