...
@QuarkusTest
class GoapGuardIntegrationTest {

  @Inject CaseInstanceCache cache;
  @Inject GuardGoapBean guardBean;

  @BeforeEach
  void setUp() {
    GuardGoapBean.executionOrder.clear();
  }

  @Test
  void goapStrategy_excludesIrrelevantBindingsFromPlan() {
    UUID caseId = guardBean.startCase(Map.of("trigger", true));
    ...
    await()
        ...
        .untilAsserted(
            () -> {
              ...
              assertThat(GuardGoapBean.executionOrder)
                  .as(
                      "GOAP should only dispatch goal-directed bindings, "
                          + "ignoring the decoy binding whose effects don't reach the goal")
                  .containsExactly("analyse", "assess", "report");
            });
  }

  @ApplicationScoped
  public static class GuardGoapBean extends CaseHub {
    ...
    public CaseDefinition getDefinition() {
      ...
      Capability capDecoy =
          Capability.builder().name("decoy").inputSchema(".").outputSchema(".").build();
          ...
          Worker.builder()
              ...
              .function(
                  new WorkerFunction.Sync<>(
                      ...
                      Map.class,
                      (input, scope) -> {
                        executionOrder.add("decoy");
                        return WorkerResult.of(Map.of("irrelevantData", true));
                      }))
              .build();
      ...
      GoapAction actDecoy = new GoapAction("decoy", Map.of(), Map.of("irrelevantData", true), 0.1);

      return CaseDefinition.builder()
          .namespace("test-goap-guard")
          .name("GOAP Guard Test")
          .version("1.0.0")
          .planningStrategy("goap")
          .capabilities(capAnalyse, capAssess, capReport, capDecoy)
          .workers(wDecoy, wReport, wAssess, wAnalyse)
          .bindings(
              Binding.builder()
                  .name("decoy")
                  .capability(capDecoy)
              ...
              Binding.builder()
                  ...
                  .on(new ContextChangeTrigger(".trigger == true"))
                  .build())
          .goapActions(List.of(actAnalyse, actAssess, actReport, actDecoy))
